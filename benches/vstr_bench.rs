...
fn run_case(name: &str, input: &str, case: fn(&str) -> usize) {
    ...
    print_result(name, start.elapsed(), checksum);
}

fn print_result(name: &str, elapsed: Duration, checksum: usize) {
    println!(
        "bench={name} iterations={ITERATIONS} elapsed_ns={} checksum={checksum}",
        elapsed.as_nanos()
    );
}
